...
- Learning Map / Course experience — spec v3 at `.docs/learning-map-spec.md`, **approved 2026-06-12, in progress**. React-first pilot, Milestones A–E. A (engine) + B (lesson player) done — see Completed Phases. Next: **Milestone C** — `/learn` course grid + `/learn/[topic]` map (SVG path, node states from `path-state.ts`, unlock animation), dashboard "Continue learning" card, first 2 diagram components. Spec deviations so far: no zod, no shiki, exit CTA → dashboard until map exists.
