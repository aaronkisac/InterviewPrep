...
**⏳ Pilot sign-off gate (Aaron):** play the React course end-to-end (EN + TR, one lesson with reduced motion on), run `pnpm lint && pnpm typecheck && pnpm test && pnpm test:e2e`. After sign-off → Phase 2 scale-out (TypeScript course first, content-only). Next batches: unit-03 Rendering & Reconciliation → unit-10 Interview Gauntlet (1 unit per batch: author → `pnpm seed` → play-test). Then E (a11y/e2e hardening + pilot retro). Spec deviations so far: no zod, no shiki.
